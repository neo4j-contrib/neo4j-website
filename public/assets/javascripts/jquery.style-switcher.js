...
(function($) {
	$.fn.styleSwitcher = function(options){		
		var defaults = {	
			slidein: true, preview: true, container: this.selector, directory: "assets/stylesheets/", useCookie: true, cookieExpires: 30, manageCookieLoad:true, linkid: "theme"
		};
		var opts = $.extend(defaults, options);
		// if using cookies and using JavaScript to load css
		if (opts.useCookie && opts.manageCookieLoad) {
			// check if css is set in cookie
			var isCookie = readCookie("style_selected_"+opts.linkid)
			if(isCookie){
				var newStyle = opts.directory + isCookie + ".css";
				$("link[id="+opts.linkid+"]").attr("href",newStyle);
				baseStyle = newStyle;
			}
			else{
		...
		var baseStyle = $("link[id="+opts.linkid+"]").attr("href");
		if(opts.preview){
			$(opts.container + " a").hover(
				...
				function () {
					$("link[id="+opts.linkid+"]").attr("href",baseStyle);
				}
			);
		}
		...
		$(opts.container + " a").click(
			function () {
				...
				baseStyle = newStyle;
				if(opts.useCookie){
					createCookie("style_selected_"+opts.linkid,this.id,opts.cookieExpires)
				}
			}
		);
